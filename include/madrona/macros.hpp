...
#define MADRONA_WARN(fmt, ...) ::fprintf(stderr, fmt __VA_OPT__(,) __VA_ARGS__)
#define MADRONA_ERROR(fmt, ...) ::fprintf(stderr, fmt __VA_OPT__(,) __VA_ARGS__)
#else
#define MADRONA_WARN(fmt, ...) ::fprintf(stderr, fmt, ##__VA_ARGS__)
#define MADRONA_ERROR(fmt, ...) ::fprintf(stderr, fmt, ##__VA_ARGS__)
#endif

// Debug logging macros - only print when NDEBUG is not defined (debug builds)
#ifndef NDEBUG
#if __cplusplus >= 202002L
#define MADRONA_DEBUG_LOG(fmt, ...) ::printf(fmt __VA_OPT__(,) __VA_ARGS__)
#else
#define MADRONA_DEBUG_LOG(fmt, ...) ::printf(fmt, ##__VA_ARGS__)
#endif
#else
